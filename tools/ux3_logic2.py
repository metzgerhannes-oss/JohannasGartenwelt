...
def line(prefix,new):
    global s
    a=s.splitlines()
    h=[i for i,x in enumerate(a) if x.startswith(prefix)]
    if len(h)!=1:
        raise SystemExit(f'{prefix}: {len(h)} matches')
    a[h[0]:h[0]+1]=new.splitlines()
    s='\n'.join(a)+('\n' if s.endswith('\n') else '')
...
if 'function friendlyGardenExplanation(' in s:
    print('Logic V3 already present')
    raise SystemExit(0)
