...
async function libraryPhoto(x,isPlant){
  var key=(isPlant?"p:":"a:")+norm(x.scientific||x.name);
  if(!key)return null;
  if(Object.prototype.hasOwnProperty.call(photoCache,key))return photoCache[key];
  photoCache[key]=null;
  try{
    var term=x.scientific||x.name;
    var u="https://api.inaturalist.org/v1/taxa/autocomplete?q="+encodeURIComponent(term)+"&locale=de&per_page=6"+(isPlant?"&taxon_id=47126":"");
    var r=await fetch(u,{headers:{Accept:"application/json"}});
    var d=r.ok?await r.json():{results:[]};
    var a=(d.results||[]).filter(function(t){return !isPlant||String(t.iconic_taxon_name||"").toLowerCase()==="plantae"});
    var exact=a.find(function(t){return norm(t.name)===norm(x.scientific)})||a.find(function(t){return t.default_photo})||a[0];
    var p=exact&&exact.default_photo||{};
    var out=(p.medium_url||p.large_url||p.square_url)?{photo:p.medium_url||p.large_url||p.square_url||"",attribution:p.attribution||"",taxonId:exact&&exact.id||null}:null;
    photoCache[key]=out;
    return out;
  }catch(e){return null}
}
async function hydrateLibraryPhotos(items,n){
  var cursor=0,workers=[];
  async function work(){
    while(cursor<items.length){
      var i=cursor++,x=items[i];
      if(x.photo)continue;
      var p=await libraryPhoto(x,kind==="plants");
      if(n!==seq)return;
      if(p&&p.photo){
        x.photo=p.photo;
        x.attribution=x.attribution||p.attribution;
        x.taxonId=x.taxonId||p.taxonId;
        var g=el("jgwLibraryGrid");
        var card=g&&g.querySelector('.jgw-library-card[data-i="'+i+'"]');
        var box=card&&card.querySelector(".jgw-library-photo");
        if(box)box.innerHTML='<img loading="lazy" src="'+esc(x.photo)+'" alt="'+esc(x.name)+'">';
      }
    }
  }
  for(var w=0;w<Math.min(4,items.length);w++)workers.push(work());
