...
function renderCalendar(){
  ...
  try{var all=tasks(),months=nextMonths(expanded?12:3);grid.innerHTML=months.map(function(mm,idx){var items=taskForMonth(all,mm.m),label=new Date(mm.y,mm.m-1,1).toLocaleDateString("de-DE",{month:"long",year:"numeric"});return '<div class="calmonth"><div class="topline"><h3>'+(idx===0?"Jetzt · ":"")+esc(label)+'</h3></div>'+(items.length?items.map(taskHtml).join(""):'<div class="muted">Keine saisonale Aufgabe startet in diesem Monat.</div>')+'</div>'}).join("");if(toggle)toggle.textContent=expanded?"Nur die nächsten 3 Monate":"Ganzes Pflegejahr anzeigen";bindExports();ensureAutoCalendar()}finally{rendering=false}
}
