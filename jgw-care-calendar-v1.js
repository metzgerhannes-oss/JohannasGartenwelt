...
function speciesCare(p){
  ...
  var isAster=/^(aster|symphyotrichum|eurybia)\b/.test(s)||/\b(aster|astern)\b/.test(n);
  if(isAster){
    push("aster-summer-chop","Astern – optionaler Sommerschnitt für mehr Fülle","✂️",5,6,"Ende Mai bis Anfang Juni Triebspitzen pinzieren oder die Triebe um etwa ein Drittel einkürzen. Das fördert Verzweigung und einen kompakteren Wuchs; die Blüte kann sich etwas nach hinten verschieben. Besonders Symphyotrichum novi-belgii reagiert oft gut, andere Astern können unterschiedlich reagieren.","RHS · Asters Growing Guide / Chelsea chop");
    var bloomRanges=ranges(p.bloomMonths);
    bloomRanges.forEach(function(r){push("aster-deadhead-"+r.start+"-"+r.end,"Astern – Verblühtes regelmäßig ausputzen","🌸",r.start,r.end,"Während der Blüte verblühte Blütenstände regelmäßig direkt entfernen. Das hält die Pflanze gepflegt und kann die Blüte bis in den Herbst verlängern.","RHS · Asters Growing Guide")});
  }
...
function renderCalendar(){
  ...
  try{var all=tasks(),months=nextMonths(expanded?12:3),seasonBox=el("seasonExports");if(seasonBox)seasonBox.innerHTML=["spring","summer","autumn","winter"].map(function(k){return '<button class="btn secondary small seasonExport" data-season="'+k+'" type="button">'+esc(seasonLabel(k))+'.ics</button>'}).join("");grid.dataset.jgwCareRendering="1";grid.innerHTML=months.map(function(mm,idx){var items=taskForMonth(all,mm.m),label=new Date(mm.y,mm.m-1,1).toLocaleDateString("de-DE",{month:"long",year:"numeric"});return '<div class="calmonth"><div class="topline"><h3>'+(idx===0?"Jetzt · ":"")+esc(label)+'</h3></div>'+(items.length?items.map(taskHtml).join(""):'<div class="muted">Keine saisonale Aufgabe startet in diesem Monat.</div>')+'</div>'}).join("");if(toggle)toggle.textContent=expanded?"Nur die nächsten 3 Monate":"Ganzes Pflegejahr anzeigen";bindExports();ensureAutoCalendar();setTimeout(function(){delete grid.dataset.jgwCareRendering},0)}finally{rendering=false}
}
...
function seasonLabel(key){var names={spring:"Frühling",summer:"Sommer",autumn:"Herbst",winter:"Winter"},a=seasonMonths(key);if(!a.length)return names[key]||key;if(key==="winter")return names[key]+" "+a[0].y+"/"+String(a[a.length-1].y).slice(-2);return names[key]+" "+a[0].y}
...
function configured(c){return /^https:\/\/[^/]+\.supabase\.co$/i.test(String(c.url||"").replace(/\/+$/,""))&&String(c.key||"").length>10&&String(c.gardenId||"").length>=6&&(/^[a-f0-9]{64}$/i.test(String(c.secretHash||""))||String(c.pin||"").length>=6)}
...
function installStyle(){if(el("jgw-care-calendar-style"))return;}
