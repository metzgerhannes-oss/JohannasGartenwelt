...
function migrateLegacyLibrary(s){
  ...
  for(const w of legacy){
    const set=setById.get(String(w.setId||''));if(!set||!w?.term||!w?.translation)continue;const subject=set.subject==='latin'?'latin':'english';const baseKey=lexicalKey(w.term,subject);if(!baseKey)continue;
    const bucketKey=`${subject}\u0000${baseKey}`;let candidates=vocabBuckets.get(bucketKey)||[];let v=null;
    if(subject==='latin'&&candidates.length){const ex=lexicalKey(w.extra||'',subject);v=candidates.find(x=>ex&&lexicalKey(x.extra||'',subject)===ex)||candidates.find(x=>!ex||!x.extra)||null;}else v=candidates[0]||null;
...
function hardenState(s){
  ...
  for(const x of rawSetsForHints){const lid=String(x?.learnerId||'');if(!subjectHints.has(lid))subjectHints.set(lid,[]);const sub=x?.subject==='latin'?'latin':'english';subjectHints.get(lid).push(sub)}
  if(s.activeLearnerId){const lid=String(s.activeLearnerId);if(!subjectHints.has(lid))subjectHints.set(lid,[]);subjectHints.get(lid).push(s.activeSubject==='latin'?'latin':'english')}
  ...
  s.books=(Array.isArray(s.books)?s.books:[]).slice(0,5000).map(raw=>{const b=raw&&typeof raw==='object'?raw:{};const isbn13=normalizeIsbn(b.isbn13||b.isbn||'');if(!isbn13)return null;const old=String(b.id||''),id=safeId(old,'book',bookUsed);if(!bookMap.has(old))bookMap.set(old,id);return {...b,id,isbn13,subject:b.subject==='latin'?'latin':'english',title:safeText(b.title,200),publisher:safeText(b.publisher,160),edition:safeText(b.edition,120),createdAt:safeText(b.createdAt||new Date().toISOString(),40),updatedAt:safeText(b.updatedAt||b.createdAt||new Date().toISOString(),40)}}).filter(Boolean);
  const bookIds=new Set(s.books.map(x=>x.id));
  const setUsed=new Set(),setMap=new Map();
  s.sets=(Array.isArray(s.sets)?s.sets:[]).slice(0,10000).map(raw=>{const x=raw&&typeof raw==='object'?raw:{};const old=String(x.id||'');const id=safeId(old,'set',setUsed);if(!setMap.has(old))setMap.set(old,id);const mappedBook=bookMap.get(String(x.bookId||''))||String(x.bookId||'');return {...x,id,learnerId:mapLearner(x.learnerId),subject:x.subject==='latin'?'latin':'english',title:safeText(x.title||'Lernset',200),schoolYear:safeText(x.schoolYear||currentSchoolYear(),24),bookId:bookIds.has(mappedBook)?mappedBook:'',bookSection:safeText(x.bookSection||x.title||'',200),testDate:safeText(x.testDate||'',10),testScopeMode:x.testScopeMode==='range'?'range':'set',testFrom:Math.max(1,Math.round(safeNumber(x.testFrom,1,100000,1))),testTo:Math.max(0,Math.round(safeNumber(x.testTo,0,100000,0))),testFormat:['target','source','mixed','dictation'].includes(x.testFormat)?x.testFormat:'target'};});
  const setIds=new Set(s.sets.map(x=>x.id));
  const vocabUsed=new Set(),vocabMap=new Map();
  s.vocabulary=(Array.isArray(s.vocabulary)?s.vocabulary:[]).slice(0,100000).map(raw=>{const v=raw&&typeof raw==='object'?raw:{};const old=String(v.id||'');const id=safeId(old,'v',vocabUsed);if(!vocabMap.has(old))vocabMap.set(old,id);return {...v,id,subject:v.subject==='latin'?'latin':'english',term:safeText(v.term,300).trim(),translation:safeText(v.translation,700).trim(),termVariants:(Array.isArray(v.termVariants)?v.termVariants:[]).slice(0,20).map(x=>safeText(x,300).trim()).filter(Boolean),translations:(Array.isArray(v.translations)?v.translations:[]).slice(0,20).map(x=>safeText(x,700).trim()).filter(Boolean),extra:safeText(v.extra,700),examples:(Array.isArray(v.examples)?v.examples:[]).slice(0,12).map(x=>safeText(x,2000)).filter(Boolean),mnemonic:safeText(v.mnemonic,1200),chunks:(Array.isArray(v.chunks)?v.chunks:[]).slice(0,30).map(x=>safeText(x,120)).filter(Boolean),sources:(Array.isArray(v.sources)?v.sources:[]).slice(-60).map(x=>{const mappedBook=bookMap.get(String(x?.bookId||''))||String(x?.bookId||'');return {kind:safeText(x?.kind||'unknown',80),setId:safeText(setMap.get(String(x?.setId||''))||x?.setId||'',120),bookId:bookIds.has(mappedBook)?mappedBook:'',at:safeText(x?.at||'',40)}}),verifiedAt:v.verifiedAt?safeText(v.verifiedAt,40):null,createdAt:safeText(v.createdAt||new Date().toISOString(),40),updatedAt:safeText(v.updatedAt||v.createdAt||new Date().toISOString(),40)};}).filter(v=>v.term&&v.translation);
  const vocabIds=new Set(s.vocabulary.map(v=>v.id));
  const bvUsed=new Set(),seenBookVocab=new Set();s.bookVocabulary=(Array.isArray(s.bookVocabulary)?s.bookVocabulary:[]).slice(0,250000).map(raw=>{const x=raw&&typeof raw==='object'?raw:{};const bookId=bookMap.get(String(x.bookId||''))||String(x.bookId||''),vocabId=vocabMap.get(String(x.vocabId||''))||String(x.vocabId||'');if(!bookIds.has(bookId)||!vocabIds.has(vocabId))return null;const section=safeText(x.section||'Lernset',200)||'Lernset',combo=`${bookId} ${section} ${vocabId}`;if(seenBookVocab.has(combo))return null;seenBookVocab.add(combo);return {id:safeId(x.id,'bv',bvUsed),bookId,vocabId,section,position:Math.max(0,Math.round(safeNumber(x.position,0,100000,0))),termOverride:safeText(x.termOverride,300),translationOverride:safeText(x.translationOverride,700),extraOverride:safeText(x.extraOverride,700),exampleOverride:safeText(x.exampleOverride,2000),createdAt:safeText(x.createdAt||new Date().toISOString(),40)}}).filter(Boolean);
  ...
  for(const raw of (Array.isArray(s.learnerVocabulary)?s.learnerVocabulary:[]).slice(0,100000)){
    const p0=raw&&typeof raw==='object'?raw:{};const old=String(p0.id||'');const learnerId=mapLearner(p0.learnerId),vocabId=vocabMap.get(String(p0.vocabId||''))||String(p0.vocabId||'');if(!vocabIds.has(vocabId))continue;
    const id=safeId(old,'w',progressUsed);const p=makeLearnerVocabulary(learnerId,vocabId,{...p0,id});const combo=`${learnerId}\u0000${vocabId}`,existing=progressByCombo.get(combo);
  ...
  const rawSetVocabulary=(Array.isArray(s.setVocabulary)?s.setVocabulary:[]).slice(0,200000);
  const linkUsed=new Set(),seenLink=new Set(),setPositions=new Map();s.setVocabulary=[];
  for(const raw of rawSetVocabulary){
    const x=raw&&typeof raw==='object'?raw:{};const setId=setMap.get(String(x.setId||''))||String(x.setId||''),vocabId=vocabMap.get(String(x.vocabId||''))||String(x.vocabId||'');if(!setIds.has(setId)||!vocabIds.has(vocabId))continue;const set=s.sets.find(z=>z.id===setId),v=s.vocabulary.find(z=>z.id===vocabId);if(!set||!v||set.subject!==v.subject)continue;const combo=`${setId}\u0000${vocabId}`;if(seenLink.has(combo))continue;seenLink.add(combo);const id=safeId(x.id,'sv',linkUsed),next=(setPositions.get(setId)||0)+1,position=Math.max(next,Math.round(safeNumber(x.position,1,100000,next)));setPositions.set(setId,position);s.setVocabulary.push(makeSetVocabulary(setId,vocabId,{...x,id,position,termOverride:safeText(x.termOverride,300),translationOverride:safeText(x.translationOverride,700),extraOverride:safeText(x.extraOverride,700),exampleOverride:safeText(x.exampleOverride,2000),source:safeText(x.source,80)}));
    const progressCombo=`${set.learnerId}\u0000${vocabId}`;if(!progressByCombo.has(progressCombo)){const p=makeLearnerVocabulary(set.learnerId,vocabId);s.learnerVocabulary.push(p);progressByCombo.set(progressCombo,p);progressIds.add(p.id)}
  }
  ...
  s.practiceTests=(Array.isArray(s.practiceTests)?s.practiceTests:[]).slice(-3000).map(t=>({...t,id:safeId(t?.id,'pt'),learnerId:mapLearner(t?.learnerId),subject:t?.subject==='latin'?'latin':'english',scopeText:safeText(t?.scopeText,700),answers:(Array.isArray(t?.answers)?t.answers:[]).slice(0,1000).map(a=>({...a,wordId:remapProgress(a?.wordId),answer:safeText(a?.answer,1000),target:safeText(a?.target,1000),prompt:safeText(a?.prompt,1000)}))}));
